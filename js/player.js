// ==========================================
// ИГРОК: ОТРИСОВКА, ДВИЖЕНИЕ, УРОН, ДРОНЫ
// ==========================================
...
Game.updatePlayer = function() {
    ...
    // 🔧 ИСПРАВЛЕННЫЙ ПОВОРОТ: базовое положение = 0 (нос вверх) + наклон
    const maxTilt = 0.5;
    const tilt = Math.max(-maxTilt, Math.min(maxTilt, dx * 0.008));
    p.targetRotation = tilt;
    p.rotation += (p.targetRotation - p.rotation) * 0.15 * DT;

    // Движение к курсору с DT для плавности
    p.x += dx * 0.1 * DT;
    p.y += dy * 0.1 * DT;
...
console.log('✅ player.js загружен (с DT для плавности)');