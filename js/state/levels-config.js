...
Game.getLevelModifiers = function(level, mode) {
    ...
    if (level <= 10) {
        return {
            speedMult: 1 + (level - 1) * 0.03,
            shootFreqMult: 1 + (level - 1) * 0.04,
            accuracyBonus: Math.min(0.1, (level - 1) * 0.01),
...
Game.LEVEL_SCRIPTS = {
    ...
    // 🔧 ИСПРАВЛЕНО: Уровень 3 БЕЗ БОССА
    // Боссы теперь только на 5, 10, 15, 20... (каждый 5-й уровень)
    3: { waves: [
        ...
        { description: 'ЭЛИТНАЯ', groups: [
            { type: 'armored', count: 2, side: 'center', role: 'attacker', canShoot: true },
            { type: 'fast', count: 3, side: 'surround', role: 'kamikaze', canShoot: false, kamikaze: true }
        ]}
    ]}
};

Game.generateWavesForLevel = function(level) {
    // 🔧 БОССЫ ТОЛЬКО НА КАЖДЫЙ 5-Й УРОВЕНЬ (5, 10, 15, 20...)
    ...
    for (let i = 0; i < waveCount; i++) {
        // Босс только на последней волне босс-уровня
