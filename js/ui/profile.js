...
Game.renderProfile = function() {
    ...
    // === XP СЕКЦИЯ (С ОТСТУПОМ ОТ АВАТАРА) ===
    const xpSection = document.createElement('div');
    xpSection.className = 'profile-xp-section';
    // 🔧 ИСПРАВЛЕНО: отступ 25px сверху, чтобы не налезал на аватар ранга
    xpSection.style.marginTop = '25px';
    ...
    // 🔧 === ХАРАКТЕРИСТИКИ КОРАБЛЯ (НЕ ТРОГАЕМ) ===
    const shipCharSection = document.createElement('div');
    shipCharSection.className = 'profile-section';
    const shipCharTitle = document.createElement('div');
    shipCharTitle.className = 'profile-section-title';
    shipCharTitle.textContent = '🚀 Характеристики корабля';
    shipCharSection.appendChild(shipCharTitle);

    const u = pd.upgrades || {};
    const shipChars = [
        { label: '❤️ HP', value: 3 + (u.shipHp || 0) },
        { label: '⚔️ Урон пуль', value: 1 + (u.shipDamage || 0) },
        { label: '⚡ Скорость пуль', value: '+' + ((u.shipBulletSpeed || 0) * 15) + '%' },
        { label: '🎯 Размер пуль', value: 'Ур.' + (u.shipBulletSize || 0) }
    ];

    const shipCharGrid = document.createElement('div');
    shipCharGrid.className = 'profile-stats-grid';
    shipChars.forEach(c => {
        const stat = document.createElement('div');
        stat.className = 'profile-stat';
        stat.innerHTML = `
        ...
        `;
        shipCharGrid.appendChild(stat);
    });
    shipCharSection.appendChild(shipCharGrid);
    content.appendChild(shipCharSection);

    // 🔧 === ХАРАКТЕРИСТИКИ ДРОНОВ (ВОССТАНОВЛЕНО) ===
    if (pd.drones && pd.drones.length > 0) {
        const dronesCharSection = document.createElement('div');
        dronesCharSection.className = 'profile-section';
        const dronesCharTitle = document.createElement('div');
        dronesCharTitle.className = 'profile-section-title';
        dronesCharTitle.textContent = '🛸 Характеристики дронов';
        dronesCharSection.appendChild(dronesCharTitle);

        const dronesCharGrid = document.createElement('div');
        dronesCharGrid.className = 'profile-stats-grid';

        pd.drones.forEach(droneId => {
            const droneType = Game.DRONE_TYPES[droneId];
            const droneUpgConf = typeof Game.DRONE_UPGRADES !== 'undefined' ? Game.DRONE_UPGRADES[droneId] : null;
            if (!droneType) return;

            const du = (pd.upgrades && pd.upgrades.drones && pd.upgrades.drones[droneId]) || {};
            const hpPerLevel = (droneUpgConf && droneUpgConf.upgrades && droneUpgConf.upgrades.hp) ? droneUpgConf.upgrades.hp.perLevel : 1;
            const dmgPerLevel = (droneUpgConf && droneUpgConf.upgrades && droneUpgConf.upgrades.damage) ? droneUpgConf.upgrades.damage.perLevel : 0.1;

            const hpVal = droneType.hp + ((du.hp || 0) * hpPerLevel);
            const dmgVal = droneType.damage + ((du.damage || 0) * dmgPerLevel);

            const icon = droneUpgConf ? droneUpgConf.icon : '🛸';
            const title = droneUpgConf ? droneUpgConf.title : droneType.name;

            let charValue = `❤️ ${hpVal} | ⚔️ ${dmgVal.toFixed(2)}`;
            if (du.fireRate) charValue += ` | ⚡ Ур.${du.fireRate}`;
            if (du.healInterval) charValue += ` | 💚 Ур.${du.healInterval}`;

            const stat = document.createElement('div');
            stat.className = 'profile-stat';
            stat.innerHTML = `
                <div class="profile-stat-label">${icon} ${title}</div>
                <div class="profile-stat-value" style="font-size:14px;">${charValue}</div>
            `;
            dronesCharGrid.appendChild(stat);
        });

        dronesCharSection.appendChild(dronesCharGrid);
        content.appendChild(dronesCharSection);
    }
...
console.log('✅ ui/profile.js загружен (отступ XP + характеристики дронов)');